...
$data['mobile'] = $this->soconfig;
		...
		if (isset($this->request->get['search'])) {
			...
			$this->document->setTitle($this->language->get('heading_title'));
				...
				$data['askquestion_status'] = $this->config->get('module_so_askquestion_status');
				...
				$show_button = false;
				if (!$this->config->get('module_so_askquestion_show_button')) {
					$show_button = true;
				}
				$data['show_button'] = $show_button;
				if (!defined ('FANCYBOX'))
					{
					$this->document->addStyle('catalog/view/javascript/so_askquestion/css/jquery.fancybox.css');
                    $this->document->addScript('catalog/view/javascript/so_askquestion/js/jquery.fancybox.js');
					define( 'FANCYBOX', 1 );
				}
				if ($data['askquestion_status'] && $data['show_category'] && $show_button) {
					$this->load->language('extension/module/so_askquestion');
					//$this->document->addStyle('catalog/view/javascript/so_askquestion/css/jquery.fancybox.css');
					$this->document->addStyle('catalog/view/javascript/so_askquestion/css/style.css');
					//$this->document->addScript('catalog/view/javascript/so_askquestion/js/jquery.fancybox.js');
					$this->document->addScript('catalog/view/javascript/so_askquestion/js/script.js');
					...
					$button_name = $this->config->get('module_so_askquestion_button_name');
					if ($button_name[$this->config->get('config_language_id')])
						$data['button_name'] = $button_name[$this->config->get('config_language_id')];
					else
						$data['button_name'] = $this->language->get('text_button_name');
					
					$data['text_askquestion_product_desc'] = $this->language->get('text_askquestion_product_desc');
				}
        ...
        if (isset($this->request->get['search']) || isset($this->request->get['tag']) || isset($this->request->get['filter_category_id']) || isset($this->request->get['filter_manufacturer_id'])) {
        ...
        'filter_manufacturer_id' => $filter_manufacturer_id,
			...
			foreach ($results as $result) {
                ...
                if ($this->config->get('module_so_product_label_status') && $this->config->get('module_so_product_label_show_search')) {
                    $productlabels = $this->model_catalog_product->getProductLabels($result['product_id']);
                    foreach ($productlabels as $label) {
                        $data['productlabels'][] = array(
                            'thumb' => $label['image'] != '' ? ($this->model_tool_image->resize($label['image'], ($label['width'] != 0 ? $label['width'] : 40), ($label['height'] != 0 ? $label['height'] : 40))) : '',
                            'type'  => $label['type'],
                            'text'  => $label['text'],
                            ...
                            'custom_class'  => $label['custom_class']
                        );
                    }                
                }             

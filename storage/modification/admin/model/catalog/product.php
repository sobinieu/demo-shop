			...
			foreach ($data['product_option'] as $product_option) {
                ...
                if (isset($data['product_label'])) {
                    foreach ($data['product_label'] as $label) {
                        $this->db->query("INSERT INTO `" . DB_PREFIX . "so_product_label` SET `product_id` = " . (int)$product_id . ", status = " . (int)$label['status'] . ", `type` = " . (int)$label['type'] . ", `position` = '" . $label['position'] . "', `image` = '" . ((int)$label['type'] == 1 ? $label['image'] : '') . "', `width` = '" . (int)$label['width'] . "', `height` = '" . (int)$label['height'] . "', `text` = '" . $label['text'] . "', show_category = " . (int)$label['show_category'] . ", show_product = " . (int)$label['show_product'] . ", `color` = '" . $label['color'] . "', `bgcolor` = '" . $label['bgcolor'] . "', `fontsize` = '" . $label['fontsize'] . "', `custom_class` = '". $label['custom_class'] ."', `start_date` = '". $label['start_date'] ."', `end_date` = '". $label['end_date'] ."'");
                    }
                }

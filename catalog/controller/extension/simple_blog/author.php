<?php
	class ControllerExtensionSimpleBlogAuthor extends Controller {
		public function index() {
			...
			$blogImgSize = '600x340';
			if($this->soconfig->get_settings('blogImgSize') != '' ){
				$blogImgSize = explode('x', $this->soconfig->get_settings('blogImgSize'));
			}
			...
			if($simple_blog_author_id) {
				...
				foreach($results as $result) {
					...
					if($result['featured_image']) {
						$image = $this->model_tool_image->resize($result['featured_image'], $blogImgSize[0],  $blogImgSize[1]);
						$featured_found = 1;
					} else if($result['image']) {
						$image = $this->model_tool_image->resize($result['image'], $blogImgSize[0],  $blogImgSize[1]);
						$featured_found = '';
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', $blogImgSize[0],  $blogImgSize[1]);
						$featured_found = '';
					}
